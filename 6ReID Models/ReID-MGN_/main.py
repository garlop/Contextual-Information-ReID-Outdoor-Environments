...
class Main():
    ...
    def save_features(self):
        ...
        print(np.shape(gallery_feature.numpy()))
        gallery_path = np.reshape(gallery_path, (-1,1))
        print(np.shape(gallery_path))

        #np.savetxt("drive/My Drive/ReID-MGN/MGNFeatures.csv", result, delimiter=",")

        outfile = '../../7Data Features/MGNFeatures.csv'
        with open(outfile, 'w') as csvfile:
          print("A")
          wtr = csv.writer(csvfile)
          titles = ['feat'+str(i) for i in range(2048)]
          ...
          print("A")
          i = 0
          for row in gallery_feature.numpy():
            print("A")
            whole_row = np.append(row,gallery_path[i])
            i = i+1
            wtr.writerow(whole_row)
