...
def get_rank_list(dist_vec, q_id, q_cam, g_ids, g_cams, rank_list_size):
  ...
  distances = []
  i = 0
  for ind, g_id, g_cam, distance in zip(sort_inds, g_ids[sort_inds], g_cams[sort_inds], dist_vec[sort_inds]):
    ...
    print("rank, after appending one element",rank_list)
    distances.append(distance)
    i += 1
    if i >= rank_list_size:
      break
  return rank_list, same_id, distances
...
def save_rank_list_to_images(rank_list, same_id, q_im_path, g_im_paths, save_path, dist_value):
	print("Entered")
	filename = '/home/dell_mserver_01/Escritorio/person-reid-triplet-loss-baseline/rankedImages.json'
	...
	with open(filename) as json_file:
		try:
			data = json.load(json_file)
    			rankings = data
			print("opening with:",len(rankings))
		except:
			rankings = []

		ranking = {}
		ranking['queryImage'] = q_im_path.replace("/home/dell_mserver_01/Documentos/Luis/PyTorch-YOLOv3/labeledNew/", "")
		imq = read_im(q_im_path)
		save_im(imq, save_path.replace(".jpg", "q.jpg"))
		count = 0;
		ranking['rankSize'] = str(len(rank_list))
		images = []
		print(rank_list)
		for ind, sid in zip(rank_list, same_id):
			im = read_im(g_im_paths[ind])
			g_image_name = re.sub("/home/dell_mserver_01/Documentos/Luis/PyTorch-YOLOv3/labeledNew/", "", g_im_paths[ind])
			print(g_image_name, "g_image_name")
			g_save_image_name = re.sub("/home/dell_mserver_01/Escritorio/logs/rank_lists/", "", save_path)
			print(g_save_image_name, "g_save_image_name")
			save_path = re.sub(g_save_image_name, g_image_name, save_path)
			images.append({'rankedImage' : save_path.replace("/home/dell_mserver_01/Escritorio/logs/rank_lists/", ""), 'ranking' : str(count+1), 'distance' : str(dist_value[count])})		
			count = count+1		
			save_im(im, save_path.replace(".jpg", "r"+str(count)+"dist"+str(dist_value[count-1])+".jpg"))
		ranking['images'] = list(images)

		rankings.append(ranking)			
		print("After appending one:",len(rankings))

	json_file.close()

	with open('rankedImages.json', 'w') as f:
		print("Before info is dumped:",len(rankings))
