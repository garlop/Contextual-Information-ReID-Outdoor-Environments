...
from tri_loss.utils.visualization import save_rank_list_to_images
...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('--dataset', type=str, default='market1501',
                        choices=['market1501', 'cuhk03', 'duke', 'custom'])
    ...
    ##Esto originalmente era 32, lo tuve que cambiar por cuestiones de memoria. Revisar despues.
...
def main():
  ...
  q_g_dist = compute_dist(q_feat, feat[is_q], type='euclidean')
  ...
  g_im_paths = [ospj(test_set.im_dir, n) for n in im_names[is_q]]

  try:
    os.remove('/home/dell_mserver_01/Escritorio/person-reid-triplet-loss-baseline/rankedImages.json')
  except:
    print("already removed")

  for dist_vec, q_id, q_cam, q_im_path, save_path in zip(
      q_g_dist, q_ids, q_cams, q_im_paths, save_paths):
    print("we executed this once")
    rank_list, same_id, distances = get_rank_list(
      dist_vec, q_id, q_cam, ids[is_q], cams[is_q], cfg.rank_list_size)

    save_rank_list_to_images(rank_list, same_id, q_im_path, g_im_paths, save_path, distances)
